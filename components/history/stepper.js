...
const StepperComp = ({
  ...
  const loopArray = useCallback(
    async (array) => {
      ...
      do {
        res = await axios.get(
          `vehicles/historyplayback?VehID=${
            query?.VehID
          }&pageNumber=${pageno}&pageSiz=${pagesize}&strDate=${
            array[0].StrDate
          }&endDate=${array[array.length - 1].EndDate}&mode=1`
        );
      ...
      const allLocations = {};
      currSelectedLocations?.forEach((re) => (allLocations[re.ID] = re));
      setAllLocations(allLocations);
      setNewAllLocations([...currSelectedLocations]);

      drawInitialVehicle(currSelectedLocations[0], locInfo, drawOptions, true);
      drawEndPointVehicle(
  ...
  const handleselectedStep = async (item, index, fromAllSteps = false) => {
    try {
      ...
      if (drawOptions.getAllLocations) {
        ...
        drawInitialVehicle(filteredData[0], locInfo, drawOptions);

        if (item.IsTrip && item.Distance) {
          setAllSteps((prev) => ({
            ...prev,
            [item.ID]: {
              ...prev[item.ID],
