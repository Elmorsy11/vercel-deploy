...
export default function ShowCam({ show, setShow, dateRange }) {
      ...
      try {
        ...
        if (loginToken !== null) {
          ...
          if (resDeviceStatus?.data?.data != null) {
            setCameraLink(
              `https://dashcam.saferoad.net/vss/apiPage/ReplayVideo.html?token=${loginToken}&deviceId=${SerialNumber}&chs=1_2_3_4&st=${startingDate}&et=${endingDate}&wnum=4&panel=1&speed=4&buffer=3000`
            );
            setloading(false);
            setHasCamera(true);
          } else {
          ...
          let resLogin = await axios.post(`vss`, {
            username: "coreuser",
          ...
          if (resLogin.status == 200 && resLogin?.data?.data !== null) {
            ...
            //this is the realtime api
            // `http://dashcam.saferoad.net:9966/vss/apiPage/RealVideo.html?token=${resLogin?.data?.data?.token}&deviceId=${SerialNumber}&buffer=2000&chs=1_2_3_4&stream=0&wnum=1&panel=1`

            if (resDeviceStatus?.data?.data != null) {

              setCameraLink(
                `https://dashcam.saferoad.net/vss/apiPage/ReplayVideo.html?token=${resLogin?.data?.data?.token}&deviceId=${SerialNumber}&chs=1_2_3_4&st=${startingDate}&et=${endingDate}&wnum=4&panel=1&speed=4&buffer=3000`
              );
              setHasCamera(true);
