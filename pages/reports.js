...
const Reports = ({ dataSideBar }) => {
  ...
  // this state to seturl incase the request take more than ten sec
  ...
  const [treeData, setTreeData] = useState(VehFullData);
  // set tree data
  // const fetchVech = async () => {
  //   setVechLoading(true);
  //   try {
  //     const res = await axios.get(`vehicles/settings?withloc=1`);

  //     setVechLoading(false);
  //     // set random serialNumbers for veh with null
  ...
  const handleApi = (fullSelectedReportData) => {
    ...
    const apiMidSlice =
      fullSelectedReportData.name !== "Active_Devices_Summary_Key"
        ? `strDate=${strDate}&EndDate=${endDate}&vehIDs=${
            vehIds ?? ""
          }&fuelPrice=${fuelPrice}${
        ...
        : `strDate=${strDate}&EndDate=${endDate}&accounts=${JSON.stringify(
            accounts
          )}`;
      ...
      if (vehChecked.length || accounts.length) {
        ...
        const vech =
          vehChecked?.length === treeData?.length || isFromDashboard
            ? `${
                api.split("?")[0]
              }?strDate=${strDate}&EndDate=${EndDate}&duration=${duration}&speed=${speed}&fuelPrice=${fuelPrice}`
            : api;
        try {
