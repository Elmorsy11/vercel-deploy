...
const ManageGroupsVehicles = ({ settBtnRef }) => {
  ...
  const [allunGroups, setunAllGroups] = useState(
    vehData?.filter((i) => i.GroupID === null)
  );
  ...
  const EditGroup = async (value) => {
    ...
    try {
      ...
      setDataoptions((prev) =>
        prev.map((option) => {
          if (option.value === editGroups.ID)
            return { ...option, label: value.Name };
          return option;
        })
      );
      ...
      setAllGroups((groups) =>
        groups.map((group) => {
          if (group.ID === editGroups.ID) return { ...group, Name: value.Name };
          return group;
        })
      );
    ...
    if (vehicleGroup.length > 0 && checkarray.length > 0) {
      try {
        ...
        if (response.status == 200) {
          ...
          toast.warning(`please sync data and refresh to see the changes`);
        }
      } catch (err) {
        setCheckInput(false);
        setcheckunasignbtn(false);
