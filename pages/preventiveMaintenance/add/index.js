...
const FormikAdd = () => {
  ...
  useEffect(() => {
    let SerialNumbers = vehChecked?.map((i) => i?.SerialNumber);
    setSelectedVehiclesData([]);
    SerialNumbers?.map((SerialNumber, i) => {
      onValue(
        ref(db, SerialNumber),
        (snapshot) => {
          let locinfo;
          if (!snapshot.hasChildren()) {
            locinfo = vehChecked[i];
          } else {
            locinfo = tolocInfo(snapshot);
          }
          setSelectedVehiclesData((prev) =>
            prev.length ? [...prev, locinfo] : [locinfo]
          );
          snapshot.exists();
        },
        (error) => {
