...
const UseTableColumns = () => {
  ...
  const handleToFixedFuleL = (params) => +params?.data?.fule_L.toFixed(2);

  const handleToFixedFuel_SR = (params) => +params?.data?.fuel_SR.toFixed(2);
  ...
  const Driving_Statistics_Per_PeriodColumn = useMemo(
    () => [
      ...
      },
      {
        ...
        field: "W_hours",
        valueGetter: (params) => {
          if (isFloat(params?.data?.W_hours)) {
            return +params?.data?.W_hours.toFixed(2);
          } else {
            return params?.data?.W_hours;
          }
        },
      },
      {
