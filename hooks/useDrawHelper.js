    ...
    (workstep, apidata, steps, drawOptions) => {
      ...
      stepMrks.forEach((step) => {
        ...
        function getContent(apidata, LocID) {
          ...
          const content = `
                <p style="margin-left: 1rem !important" ><i></i> Display Name : ${
                  si?.DisplayName
                }</p>
                <p style="margin-left: 1rem !important" ><i class="${
                  Resources.Icons?.VehicleStatus
                } pe-1"></i> VehicleStatus : ${vs}</p>
                <p style="margin-left: 1rem !important" ><i class="${
                  Resources.Icons?.RecordDateTime
                } pe-1"></i> ${moment(si?.RecordDateTime)
            .utc()
            .local()
            .format("LL hh:mm:ss a")}</p>
                <p style="margin-left: 1rem !important" >${si?.Latitude} , ${
            si?.Longitude
          }</p>
                <p style="margin-left: 1rem !important" ><i class="${
                  Resources.Icons?.Direction
                } pe-1"></i> Direction : ${si?.Direction}</p>
                <p style="margin-left: 1rem !important" ><i class="${
                  Resources.Icons?.Speed
                } pe-1"></i> ${
            si?.Speed
          } <span style="font-size: 0.6rem;">KM/h</span></p>
                <p style="margin-right: 1rem !important; margin-left: 1rem !important; margin-bottom: 1rem !important" ><i class="${
                  Resources.Icons?.Address
                } pe-1"></i> ${si?.Address}</p>`;
          return content;
        }
