import { locConfigModel, locDataModel, Date2KSA, WeightVoltToKG, isDateExpired } from "../helpers/helpers";
...
const StreamHelper = () => {
  ...
  const getAddress = async (VehFullData, _SerialNumber, _div = null, _updateList = true, trial = 0) => {
    ...
    try {
      var getJson = Object.assign({}, { Address: data, RecordDateTime: pubLocInfo.RecordDateTime, Longitude: pubLocInfo.Longitude, Latitude: pubLocInfo.Latitude });
    }
    catch (err) { }
  };
  ...
  const aggregate = (VehFullData, newInfo, oldInfo, _initial) => {
    ...
    newInfo.Duration +=
      newInfo.VehicleStatus == oldInfo.VehicleStatus && Number.isInteger(oldInfo.Duration)
        ? oldInfo.Duration
        : 0;
  ...
  const checkNewOfflines = (VehFullData) => {
    var newOfflines = VehFullData.filter(x => x?.VehicleStatus != 5 || x?.VehicleStatus != 600).filter(x => isDateExpired(x));
