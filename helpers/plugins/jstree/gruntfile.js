...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      libs: {
        files: [{ expand: true, cwd: "libs/", src: ["*"], dest: "dist/libs/" }],
      },
      docs: {
        files: [
          {
    ...
    uglify: {
      options: {
        banner:
          '/*! <%= pkg.title || pkg.name %> - v<%= pkg.version %> - <%= grunt.template.today("yyyy-mm-dd") %> - (<%= _.map(pkg.licenses, "type").join(", ") %>) */\n',
      ...
      production: {
        ...
        files: {
          ...
          "dist/themes/default-dark/style.min.css":
            "src/themes/default-dark/style.less",
        },
      },
      development: {
        files: {
          ...
          "src/themes/default-dark/style.css":
            "src/themes/default-dark/style.less",
          "dist/themes/default-dark/style.css":
            "src/themes/default-dark/style.less",
        },
      },
    },
    ...
    grunt.file.copy(s, d, {
      process: function (contents) {
        contents = contents.replace(
          /\s*if\(\$\.jstree\.plugins\.[a-z]+\)\s*\{\s*return;\s*\}/gi,
          ""
        );
        contents = contents.replace(/\/\*globals[^\/]+\//gi, "");
  ...
  grunt.registerMultiTask("dox", "Generate dox output ", function () {
    ...
    exec(
      formatter + ' < "dist/jstree.js" > "docs/jstree.json"',
      { maxBuffer: 5000 * 1024 },
