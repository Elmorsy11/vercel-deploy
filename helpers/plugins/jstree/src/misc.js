...
(function ($, undefined) {
  ...
  $.jstree.plugins.realcheckboxes = function (options, parent) {
    this.bind = function () {
      ...
      this.element.on(
        "changed.jstree uncheck_node.jstree check_node.jstree uncheck_all.jstree check_all.jstree move_node.jstree copy_node.jstree redraw.jstree open_node.jstree ready.jstree loaded.jstree",
        function () {
          // only if undetermined is in setting
          if (this._data.realcheckboxes.uto) {
            clearTimeout(this._data.realcheckboxes.uto);
          }
          this._data.realcheckboxes.uto = setTimeout(
            this._realcheckboxes.bind(this),
            50
          );
        }.bind(this)
      );
      ...
      if (obj) {
        ...
        if (tmp) {
          for (i = 0, j = tmp.childNodes.length; i < j; i++) {
            if (
              tmp.childNodes[i] &&
              tmp.childNodes[i].className &&
              tmp.childNodes[i].className.indexOf("jstree-checkbox") !== -1
            ) {
    ...
    this._realcheckboxes = function () {
      ...
      $(".jstree-realcheckbox").each(function () {
        this.checked =
          (!ts &&
            this.parentNode.parentNode.className.indexOf("jstree-checked") !==
              -1) ||
          (ts &&
            this.parentNode.parentNode.className.indexOf("jstree-clicked") !==
              -1);
        this.indeterminate =
          this.parentNode.className.indexOf("jstree-undetermined") !== -1;
        this.disabled =
          this.parentNode.parentNode.className.indexOf("disabled") !== -1;
      });
    };
  };
})(jQuery);
  ...
  $.jstree.plugins.stackedicon = function (options, parent) {
    ...
    this.redraw_node = function (obj, deep, is_callback, force_render) {
      ...
      if (obj) {
        ...
        if (tmp) {
          if (
            this._model.data[obj.id].state.icons &&
            this._model.data[obj.id].state.icons.length
          ) {
            icon = _s.cloneNode(false);
            for (
              i = 0, j = this._model.data[obj.id].state.icons.length;
              i < j;
              i++
            ) {
...
(function ($, undefined) {
  ...
  $.jstree.plugins.datamodel = function (options, parent) {
    ...
    this._load_node = function (obj, callback) {
      ...
      var nd =
        obj.id === "#"
          ? this.settings.core.data
          : this._data.datamodel[obj.id].getChildren(
              function (nodes) {
    ...
    this.patch_data = function () {
      ...
      if ($.vakata.is_function(parentsloadSettings)) {
        ...
        if (!(parentsloadSettings.url || parentsloadSettings.data)) {
          callError(
            "02",
            'The "data.url" or "data.data" must be presented in configuration'
          );
          return;
        }
        ...
        callError(
          "03",
          'The appropriate "data.url" or "data.data" must be presented in configuration'
        );
      }
    };
...
(function (factory) {
  ...
  if ($.jstree.plugins.conditionaldeselect) {
    return;
  }
  $.jstree.defaults.conditionaldeselect = function () {
    return true;
  };
...
(function (factory) {
  ...
  if ($.jstree.plugins.conditionalclose) {
    return;
  }
  $.jstree.defaults.conditionalclose = function () {
    return true;
  };
  $.jstree.plugins.conditionalclose = function (options, parent) {
    ...
    this.close_node = function (obj, animation) {
      if (
        this.settings.conditionalclose.close.call(this, this.get_node(obj), e)
      ) {
...
(function (factory) {})(function ($, jstree, undefined) {
  "use strict";
  $.jstree.plugins.node_customize = function (options, parent) {
    this.redraw_node = function (obj, deep, callback, force_draw) {
      var el = parent.redraw_node.apply(this, arguments);
  ...
  $.jstree.plugins.material = function (options, parent) {
    ...
    this.redraw_node = function (obj, deep, is_callback, force_render) {
      ...
      if (obj) {
        ...
        if (tmp) {
          if (
            this._model.data[obj.id].icon &&
            this._model.data[obj.id].icon.length
          ) {
